...
for code in orig; do
        ...
        ../../remote-throughput.py --medium $r_medium --code $code --time $r_time --run $i --cluster $r_cluster --sync no --user $r_user --workload $workload --num_nodes 5 --target_system_name $r_system --sync_rep_factor 5 --num_clients $n --leader_reads yes --batch 20
        sleep 2
done
done
done                            
...
for code in rtop; do
        # set the batch parameter to 20 batched Paxos...this is the value for which paxos performed the best
        ../../remote-throughput.py --medium $r_medium --code $code --time $r_time --run $i --cluster $r_cluster --sync no --user $r_user --workload $workload --num_nodes 5 --target_system_name $r_system --sync_rep_factor 5 --num_clients $n --leader_reads yes --batch 64
        sleep 2
done
done
done
